...
def payment_success(request, order_id):
    if request.user.is_authenticated:
        order = get_object_or_404(Order, id=order_id, created_by=request.user, is_paid=True)
    else:
...
def cancel_order_view(request, order_id):
    ...
    if order.status == 'cancelled':
        if request.headers.get('HX-Request'):
            return HttpResponse('<div class="error-message">Order is already cancelled.</div>')
        messages.warning(request, 'Order is already cancelled.')
        return redirect('user_account')
    
    if order.status in ['shipped', 'delivered']:
        if request.headers.get('HX-Request'):
            return HttpResponse('<div class="error-message">Cannot cancel shipped/delivered order.</div>')
        ...
        try:
            ...
            if order.payment_intent:
                ...
                if refund.status not in ['pending', 'succeeded']:
                    ...
                    if request.headers.get('HX-Request'):
                        return HttpResponse('<div class="error-message">Refund failed.</div>')
            ...
            transaction.savepoint_rollback(sid)
            if request.headers.get('HX-Request'):
                return HttpResponse(f'<div class="error-message">Error processing refund: {str(e)}</div>')
    ...
    if refund_id:
        success_message = f'Order cancelled and refund initiated. Refund ID: {refund_id}'
    else:
        success_message = 'Order cancelled successfully.'
    
    if request.headers.get('HX-Request'):
        return render(request, 'userprofile/partials/order_card.html', {
            'order': order,
            'success_message': success_message
        })
    else:
        if refund_id:
            messages.success(request, f'Order cancelled and refund initiated. Refund ID: {refund_id}')
        else:
